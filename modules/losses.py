...
from modules.transforms import unnorm, kspace_to_mri, normalize_to_minus_one_one
...
def ssim(
    ...
    for slice_num in range(gt.shape[0]):
        ...
        ssim = ssim + structural_similarity(
            gt[slice_num], pred[slice_num], data_range=data_range, win_size=7
        )
...
class LPIPSWithDiscriminator(nn.Module):
    def __init__(self, disc_start, logvar_init=0.0, kl_weight=1.0, pixelloss_weight=1.0,
        ...
        self.disc_factor = torch.tensor(disc_factor)
    ...
    def forward(self, inputs, reconstructions, posteriors, optimizer_idx,
                global_step, norm_mean, norm_std, target, last_layer=None, cond=None, split="train",
                weights=None):
        ...
        if self.perceptual_weight > 0:
            ...
            inputs_img = normalize_to_minus_one_one(target.repeat(1, 3, 1, 1))
            reconstructions_img = normalize_to_minus_one_one(reconstructions_img.repeat(1, 3, 1, 1))
        ...
        if optimizer_idx == 0:
            ...
            d_weight = d_weight.to(g_loss)
            disc_factor = adopt_weight(self.disc_factor, global_step, threshold=self.discriminator_iter_start)
            if isinstance(disc_factor, float):
                disc_factor = torch.tensor(disc_factor)
            disc_factor = disc_factor.to(g_loss)
...
class SSIMAndLPIPSWithDiscriminator(nn.Module):
    def __init__(self, disc_start, logvar_init=0.0, kl_weight=1.0, pixelloss_weight=1.0,
                 disc_num_layers=3, disc_in_channels=3, disc_factor=1.0, disc_weight=1.0,
    ...
    def forward(self, inputs, reconstructions, posteriors, optimizer_idx,
                global_step, norm_mean, norm_std, max_value, target, last_layer=None, cond=None, split="train",
                weights=None):
        rec_loss = torch.abs(inputs.contiguous() - reconstructions.contiguous())
        if self.perceptual_weight > 0:
            # TODO: here we have to calculate the perceptual loss on the image
        ...
        if self.ssim_weight > 0:
            reconstructions_unnormed = unnorm(reconstructions, norm_mean, norm_std).permute(0, 2, 3, 1).contiguous()
            reconstructions_img = kspace_to_mri(reconstructions_unnormed).unsqueeze(1)
            s_loss = self.ssim_loss(reconstructions_img, target, max_value)
            rec_loss = rec_loss + self.ssim_weight * s_loss
